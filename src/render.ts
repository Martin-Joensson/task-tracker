import type { Task, Status, Priority, SortBy } from "./types.js";
import {
  addTask,
  clearTasks,
  getTasks,
...
let taskGrid: HTMLElement | null = null;
...
export function renderHeader(updateUI: () => void) {
  if (!header) return;
...
export function createTaskCard(task: Task, updateUI: () => void): HTMLElement {
...
export function renderTasks(updateUI: () => void) {
  if (!taskGrid) return;
...
export function renderAddBar(
  ...
  form.addEventListener("submit", (event) => {
    ...
    addTask(taskName, select.value as any);
...
export function renderFooter(lastSaved: string | null, updateUI: () => void) {
